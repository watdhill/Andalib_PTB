...
exports.createPeminjaman = async (req, res) => {
  ...
  if (!nim) {
    return res
      .status(400)
      .json({ success: false, message: "NIM anggota wajib diisi" });
  }
  if (!bukuId) {
    return res
      .status(400)
      .json({ success: false, message: "ID buku wajib diisi" });
  }
  if (!jatuhTempo) {
    return res
      .status(400)
      .json({ success: false, message: "Tanggal jatuh tempo wajib diisi" });
  }
...
exports.updatePeminjaman = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Gagal mengupdate peminjaman" });
  }
};
...
exports.uploadKrsImage = async (req, res) => {
  ...
  if (!req.file) {
    return res
      .status(400)
      .json({ success: false, message: "File KRS tidak ditemukan" });
  }
